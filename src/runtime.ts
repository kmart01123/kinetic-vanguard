...
export function clientRuntime(model: any): void {
  ...
  function renderBlock(parent: HTMLElement, block: any): void {
    if(block.type==="example_play_section"){const section=document.createElement("section");section.className=`example-play-section example-play-section--${block.discipline}`;section.dataset.discipline=block.discipline;const heading=document.createElement("h3");heading.className="example-play-section__heading";heading.id=`example-play-${block.discipline}`;block.heading.forEach((node:any)=>appendInline(heading,node));section.setAttribute("aria-labelledby",heading.id);const card=document.createElement("div");card.className="example-play-section__card";const title=document.createElement("h4");title.className="example-play-section__title";block.title.forEach((node:any)=>appendInline(title,node));const body=document.createElement("div");body.className="example-play-section__body";block.body.forEach((child:any)=>renderBlock(body,child));card.append(title,body);section.append(heading,card);parent.append(section);return;}
  ...
  function renderTopic(focusEntity?: string): void {
    ...
    for(const entityId of current.entity_ids){const entity=entities.get(entityId) as any;const article=document.createElement("article");article.id=`entity-${entity.id}`;const heading=document.createElement("h2");heading.textContent=entity.title;heading.tabIndex=-1;heading.dataset.sourcePath=`entities.${entity.id}.title`;article.append(heading);
      ...
      if(entity.content.every((block:any)=>block.type==="example_play_section")){const sections=document.createElement("div");sections.className="example-play-sections";entity.content.forEach((block:any)=>renderBlock(sections,block));article.append(sections);}else entity.content.forEach((block:any)=>renderBlock(article,block));renderExampleTurns(article,entity);main.append(article);}
