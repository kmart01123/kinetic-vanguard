...
import { deriveCalculatorProjection } from "./mechanics-selectors.js";
...
export interface HarnessProjection {
  projection_version:"1.11.0";
  authority_path:string;
  authority_sha256:string;
  rules_version:string;
  schema_version:string;
  supported_level_range:{minimum:number;maximum:number};
  progressions:{proficiency_bonus:CalculatorLevelBand[];psi_points:CalculatorLevelBand[];psionic_focus:CalculatorLevelBand[];manifested_strike_die:CalculatorLevelBand[];tier_minimum_levels:CalculatorProjection["tier_minimum_levels"]};
  core:Pick<CalculatorProjection["harness_mechanics"],"action_economy"|"manifested_strike"|"overload"|"psionic_apex">;
  disciplines:CalculatorProjection["harness_mechanics"]["disciplines"];
  features:Array<HarnessFeatureRule&{title:string;minimum_level:number;psi_cost:number;activation:string;damage_delivery:string|null;damage_tiers:NonNullable<CalculatorProjection["features"][number]["tiers"]>;advanced_training:boolean;selectable_advanced_training:boolean}>;
}
...
export async function createHarnessProjection(authorityPath="KineticVanguard.yaml"):Promise<HarnessProjection>{
  ...
  const authority=loaded.authority,calculator=deriveCalculatorProjection(authority),harness=calculator.harness_mechanics;
  ...
  return {
    projection_version:"1.11.0",authority_path:resolve(authorityPath),authority_sha256:sha256(loaded.sourceBytes),rules_version:authority.rules_version,schema_version:authority.schema_version,
