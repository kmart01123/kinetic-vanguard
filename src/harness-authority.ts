...
export interface HarnessProjection {
  ...
  core:Authority["calculator"]["harness_mechanics"] extends infer H ? H extends {manifested_strike:infer M;overload:infer O}?{manifested_strike:M;overload:O}:never:never;
  disciplines:Authority["calculator"]["harness_mechanics"]["disciplines"];
  features:Array<HarnessFeatureRule&{minimum_level:number;psi_cost:number;activation:string;damage_delivery:string|null;damage_tiers:Authority["calculator"]["features"][number]["tiers"]}>;
}
...
export async function createHarnessProjection(authorityPath="KineticVanguard.yaml"):Promise<HarnessProjection>{
  ...
  const features=harness.feature_rules.map(rule=>{
    ...
    return {...structuredClone(rule),minimum_level:entity.level,psi_cost:entity.psi_cost,activation:entity.activation??"passive",damage_delivery:damage?.delivery??null,damage_tiers:structuredClone(damage?.tiers??[])};
  });
  return {
    ...
    core:{manifested_strike:structuredClone(harness.manifested_strike),overload:structuredClone(harness.overload)},disciplines:structuredClone(harness.disciplines),features
  };
}
