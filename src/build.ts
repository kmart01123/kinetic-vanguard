...
export async function executeBuild(profileName:BuildProfile,outputRoot="artifacts") {
  ...
  if(profileName==="release"){
    ...
    const correctness=YAML.parse(await readFile("tests/filtered-search-correctness.yaml","utf8"));if(correctness.review_status!=="accepted")diagnostics.push({severity:"error",code:"filter.correctness_unreviewed",message:"Filtered-search correctness corpus lacks independent accepted review"});
    if(!migrationResult.state.manifest.migration_acceptance)diagnostics.push({severity:"error",code:"migration.acceptance_missing",message:"Migration acceptance evidence is absent"});
    diagnostics.push({severity:"error",code:"environment.mutable_base_image",message:"Revision-21 devcontainer still uses a mutable base-image tag; release requires an immutable digest"});
  }
