...
export async function executeBuild(profileName:BuildProfile,outputRoot="artifacts",authorityPath="KineticVanguard.yaml") {
  ...
  const html=renderHtml({authority:loaded.authority,ui,derived,policy,filterIndex,runtimeSource,releaseStatus:profile.releaseStatus,authorityHash:sha256(loaded.sourceBytes)});
