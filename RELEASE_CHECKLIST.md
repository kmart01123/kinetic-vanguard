...
- [ ] Run the focused current-development validation commands `npm run harness:validate` and `npm run test:harness`; together they protect the maintained damage evaluator/report contracts and the static Control Authority v2 and `ControlTarget` input contracts without running the full analytical universe. Neither command evaluates control or publishes a control result.
...
- [ ] Keep the generated README region limited to one single-target damage heat table. Confirm current-development documentation and release gates intentionally expose no maintained v14.2 control evaluator or current v14.2 control result. Treat retained Control Authority v2 and `ControlTarget` data as static inputs only, not a benchmark methodology or execution runtime; a minimum execution contract and named-condition runner require separate approval and remain unimplemented future work. This capability gap is not by itself a release blocker.
...
- [ ] Scan every package script and every CI/release workflow. Confirm none invokes the retired Control Reliability stack or the superseded control execution runtime, all retired runtime paths are absent, and `control:engine:validate` and `control:engine:fixtures` are absent. Confirm no compatibility or fallback runtime, dormant control CLI, current control-result surface, or current-version control publication wiring remains.
