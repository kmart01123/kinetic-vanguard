...
- [ ] When one of those triggers applies, record the trigger, run `npm run readme:damage`, investigate and commit the synchronized numerical diff, and run `npm run readme:damage:check`. The check must use fresh exact analytical evaluation without a tracked golden result or Monte Carlo sampling. Otherwise, do not recompute an unchanged analytical universe or rewrite its evidence merely for release metadata.
