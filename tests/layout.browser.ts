...
test("Psi Cost Reference scrolls on mobile and fits every column in print",async()=>{
  ...
  for(const engine of desktopBrowsers){
    ...
    try{
      ...
      await page.emulateMedia({media:"print"});await page.setViewportSize({width:1366,height:900});await page.goto(url);
      const printLayout=await page.evaluate(()=>{const table=document.querySelector<HTMLElement>("#entity-subclass_feature_reference .quick-reference-table")!;const wrapper=table.closest<HTMLElement>(".table-scroll")!;const wrapperRect=wrapper.getBoundingClientRect(),lastColumn=table.querySelector("th:last-child")!.getBoundingClientRect();return{overflowX:getComputedStyle(wrapper).overflowX,tableWidth:table.getBoundingClientRect().width,wrapperWidth:wrapperRect.width,lastColumnContained:lastColumn.left>=wrapperRect.left&&lastColumn.right<=wrapperRect.right,documentOverflow:document.documentElement.scrollWidth-document.documentElement.clientWidth};});
