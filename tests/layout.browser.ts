...
test("Psi Cost Reference remains horizontally scrollable at mobile width",async()=>{
  ...
  for(const engine of desktopBrowsers){
    ...
    try{
      ...
      assert.ok(layout,engine.name+" reference table");assert.deepEqual(layout.headers,["Level","Feature","Discipline","Psi","Activation","Duration"]);assert.equal(layout.wrapperTabIndex,"0");assert.ok(["auto","scroll"].includes(layout.overflowX));assert.equal(layout.scrollable,true);assert.equal(layout.documentOverflow,0);assert.equal(layout.levelWhiteSpace,"nowrap");assert.equal(layout.psiWhiteSpace,"nowrap");
    }finally{await browser.close();}
  }
});
