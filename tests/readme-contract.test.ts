...
test("README exposes one structurally valid headline balance snapshot",async()=>{
  ...
  assert.match(region,/`eligible\/roster` means \*\*legally eligible targets \/ total maintained benchmark targets\*\*/);assert.match(region,/An ineligible target remains in the aggregate denominator.*`CU = 0` and `delivery = 0%`/s);assert.match(region,/Do not divide only by eligible targets.*Eligible-only averaging would hide practical restrictions/s);assert.match(region,/\*\*Instructional example \(not a published scenario\):\*\*.*\(9 × 0\.80 \+ 3 × 0\) \/ 12 = 0\.60 = 60%.*eligible-only 80% is not the roster-wide result/s);
