...
test("README exposes one structurally valid headline balance snapshot",async()=>{
  ...
  for(const heading of ["Balance benchmark snapshot","Single-Target Damage","Control Value","Kinetic Vanguard mean Control Value","Kinetic Vanguard control catalog","Control coverage exceptions","Benchmark roster, effectiveness, and coverage","How Control Value is calculated","Worked example: Sap-style next-attack Disadvantage","Worked example: Stunned","Control Reliability — delivery diagnostic","Kinetic Vanguard mean Reliability","Why Control Value and Reliability can disagree","Control methodology"])assert.match(region,new RegExp("^#{2,4} "+heading.replace(/[.*+?^$(){}|[\]\\]/g,"\\$&")+"$","m"));
