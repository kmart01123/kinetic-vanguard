...
test("README exposes one structurally valid headline balance snapshot",async()=>{
  ...
  assert.equal(occurrences(region,"| Rider / form | L7 | L11 | L15 | L20 |"),4);assert.match(region,/Forked Lightning — T2 — primary/);assert.match(region,/Forked Lightning — T2 — secondary/);assert.match(region,/ineligible targets remain in the denominator with zero contribution/);
