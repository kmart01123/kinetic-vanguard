...
test("README exposes one structurally valid headline balance snapshot",async()=>{
  ...
  for(const heading of ["Balance benchmark snapshot","Single-Target Damage","Control Value","Kinetic Vanguard mean Control Value","Kinetic Vanguard control catalog","How Control Value is calculated","Worked example: Sap-style next-attack Disadvantage","Worked example: Stunned","Control Reliability — delivery diagnostic","Kinetic Vanguard mean Reliability","Why Control Value and Reliability can disagree","Control methodology"])assert.match(region,new RegExp("^#{2,4} "+heading.replace(/[.*+?^$(){}|[\]\\]/g,"\\$&")+"$","m"));
  ...
  assert.equal(occurrences(region,"| Rider / form | L7 | L11 | L15 | L20 |"),4);assert.match(region,/Forked Lightning — T2 — primary/);assert.match(region,/Forked Lightning — T2 — secondary/);
  assert.match(region,/\*\*Cell format:\*\* `CU · delivery · eligible\/roster`/);assert.match(region,/`0\.143 CU · 95\.00% · 12\/12` means `0\.143 CU` average Control Value and `95\.00%` average initial control-delivery probability.*`12\/12` means the exact form is legally eligible against all 12 targets/s);assert.match(region,/ratio is \*\*eligible targets \/ roster targets\*\*/);
  assert.match(region,/`9\/12`, only 9 of 12 targets are legally eligible.*other 3.*remain in the roster denominator and contribute `0 CU` and `0% delivery`/s);assert.match(region,/size, creature-type, immunity\/effect-eligibility, and other explicit legality restrictions reduce the roster-wide means instead of being hidden by eligible-only averaging/);assert.match(region,/Eligibility is not a save result, hit count, successful application count, or probability/);
