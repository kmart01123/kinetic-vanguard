...
test("README exposes one structurally valid headline balance snapshot",async()=>{
  ...
  for(const heading of ["Balance benchmark snapshot","Single-Target Damage","Control Value","Kinetic Vanguard mean Control Value","Kinetic Vanguard control catalog","Control coverage exceptions","Benchmark roster, effectiveness, and coverage","How Control Value is calculated","Worked example: Sap-style next-attack Disadvantage","Worked example: Stunned","Control Unit primitive pricing rubric","Maintained transform definitions","How movement control is normalized","Context-dependent and unpriced control primitives","Control Value normalization rules","Control Reliability — delivery diagnostic","Kinetic Vanguard mean Reliability","Why Control Value and Reliability can disagree","Control methodology"])assert.match(region,new RegExp("^#{2,4} "+heading.replace(/[.*+?^$(){}|[\]\\]/g,"\\$&")+"$","m"));
  ...
  assert.match(region,/\*\*Cell format:\*\* `CU · delivery · effective\/roster`/);assert.match(region,/`0\.143 CU · 95\.00% · 12\/12` means `0\.143 CU` average Control Value and `95\.00%` average initial control-delivery probability.*at least one modeled control consequence.*structural restrictions, immunities, and effect dependencies/s);
  assert.match(region,/`12\/12 effective` does \*\*not\*\* mean 100% delivery or that every consequence works.*`10\/11 effective` means one of the 11 creatures cannot receive any modeled control/s);assert.match(region,/partial-effect exception.*Coverage is not a save result, hit count, successful application count, CU threshold, pricing state, or delivery probability/s);assert.doesNotMatch(region,/eligible\/roster/);
  assert.match(region,/`Unpriced` retains measurable delivery and effectiveness coverage without reporting zero CU/);assert.match(region,/`No modeled control` means `0\.000 CU` and no control delivery \(`—`\)/);assert.match(region,/`N\/A` means the exact form is unavailable at that level/);
  const exceptionsHeading="### Control coverage exceptions",methodologyHeading="### Benchmark roster, effectiveness, and coverage",methodologyLink="[Benchmark roster, effectiveness, and coverage](#benchmark-roster-effectiveness-and-coverage)";assert.equal(occurrences(region,exceptionsHeading),1);assert.equal(occurrences(region,methodologyHeading),1);assert.equal(occurrences(region,methodologyLink),1);
  assert.match(region,/Structural legality remains an internal prerequisite.*`target_is_eligible\(\)`.*maximum-size and required-creature-type restrictions.*`effective\/roster` coverage/s);assert.match(region,/immunity can instead remove one or more consequences.*partially effective and remains in the coverage numerator.*every modeled consequence is nullified, the target is ineffective/s);assert.match(region,/Effective coverage is descriptive metadata, not a success roll, CU threshold, pricing state, delivery probability, or alternate averaging population.*ineffective target remains in the aggregate denominator/s);assert.match(region,/Do not divide only by effective targets.*Effective-only averaging would hide practical restrictions/s);assert.match(region,/\*\*Instructional example \(not a published scenario\):\*\*.*9 effective targets.*3 ineffective targets.*\(9 × 0\.80 \+ 3 × 0\) \/ 12 = 0\.60 = 60%.*effective-only 80% is not the roster-wide result/s);
  const catalogRegion=region.slice(region.indexOf("### Kinetic Vanguard control catalog"),region.indexOf(exceptionsHeading));const catalogRows=catalogRegion.split("\n").filter(line=>/^\| (?!Rider \/ form|---)/.test(line));assert.equal(catalogRows.length,67);
  const exceptionRegion=region.slice(region.indexOf(exceptionsHeading),region.indexOf(methodologyHeading));assert.match(exceptionRegion,/Cryokinesis — Snow Chains — T0 \| Fighter 7 \| Air Elemental \| Partial \| immune to Restrained; Speed 0 remains effective/);assert.match(exceptionRegion,/Cryokinesis — Snow Chains — T1 \| Fighter 7 \| Air Elemental \| Partial \| immune to Restrained; Speed 0 and Reaction denial remain effective/);
  ...
  assert.match(region,/opportunity-normalized synthetic example.*1\.00 expected exposure independently.*target_turn_window.*reaction_window.*save_opportunity.*incoming_attack_opportunity/s);
  assert.match(region,/Real Stunned benchmark rows do \*\*not\*\* automatically equal 2\.25 CU/);
  assert.match(region,/Stunned does \*\*not\*\* gain Speed 0/);
  const rubricRegion=region.slice(region.indexOf("### Control Unit primitive pricing rubric"),region.indexOf("### Context-dependent and unpriced control primitives"));const pricingRows=rubricRegion.split("\n").filter(line=>/^\| `[^`]+` \| `[^`]+` \| `(?:candidate|context_required|unsupported)` \| \d+\.\d{2} CU \| `[^`]+` \|$/.test(line));assert.equal(pricingRows.length,19);
  ...
  const unpricedRegion=region.slice(region.indexOf("### Context-dependent and unpriced control primitives"),region.indexOf("### Control Value normalization rules"));const unpricedRows=unpricedRegion.split("\n").filter(line=>/^\| `[^`]+` \| `[^`]+` \| `(?:context_required|unsupported)` \|/.test(line));assert.equal(unpricedRows.length,32);assert.match(unpricedRegion,/lacks a trustworthy required magnitude, timing, or placement\/exposure basis/);
  const normalizationRegion=region.slice(region.indexOf("### Control Value normalization rules"),region.indexOf("### Control Reliability — delivery diagnostic"));for(const rule of ["Duplicates","Disjoint sequential stages","Action-economy dominance","Specified Action interaction","Attack impairment","Save impairment","Movement dominance","Correlated flat mobility","Partial overlap","Unrelated consequences"])assert.equal(occurrences(normalizationRegion,`| ${rule} |`),1);
