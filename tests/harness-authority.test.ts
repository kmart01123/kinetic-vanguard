...
test("damage projection reads the real authority and joins mechanics by stable entity ID",async()=>{
  const projection=await createDamageHarnessProjection();
  assert.equal(projection.projection_version,"1.0.0");assert.equal(projection.rules_version,"14.2.0");assert.equal(projection.schema_version,"3.0.0");assert.match(projection.authority_path,/\/KineticVanguard\.yaml$/);
  ...
  assert.deepEqual(projection.disciplines,[
    {id:"pyrokinesis",damage_type:"fire",signature_save:"dexterity",graze_damage:"psionic_ability_modifier"},
    {id:"cryokinesis",damage_type:"cold",signature_save:"constitution"},
    {id:"psychokinesis",damage_type:"force",signature_save:"strength"},
    {id:"electrokinesis",damage_type:"lightning",signature_save:"charisma"}
  ]);
  ...
  const flare=projection.features.find(item=>item.entity_id==="flare")!;assert.equal(flare.minimum_level,15);assert.equal(flare.psi_cost,3);assert.equal(flare.damage_tiers.length,3);
  const serialized=JSON.stringify(projection);
  for(const retiredField of ["control_tiers","control_outcomes","damage_required","maximum_size","replaces_mastery"])assert.equal(serialized.includes(`"${retiredField}"`),false,retiredField);
});
...
test("control authority v2 projection is explicit, complete, deterministic, and does not coerce legacy labels",async()=>{
  ...
  assert.deepEqual(first,second);assert.equal(first.projection_version,"2.0.0");assert.equal(first.schema_version,"3.0.0");assert.equal(first.rules_version,"14.2.0");
...
test("official damage-authority source is positive input while imports and generated outputs remain excluded",async()=>{
  ...
  for(const required of ["src/harness-authority.ts","harness/authority.py","harness/damage_harness.py","harness/damage_report.py","harness/readme_damage.py","harness/config/benchmark.json","harness/comparators/fighter-subclasses.json","harness/data/srd_targets.csv","harness/provenance/damage-review.json","harness/tests/test_authority_v2.py","harness/tests/test_authority_v2_parity.py","harness/tests/test_harness.py","harness/tests/test_readme_damage.py","tests/control-authority-v2-parity.test.ts","tests/fixtures/control-authority-v2-parity.json"])assert.ok(paths.includes(required),required);
  for(const retired of ["harness/control_harness.py","harness/readme_matrices.py","harness/comparison_report.py","harness/provenance/legacy-import.json","harness/tests/test_readme_matrices.py"])assert.equal(paths.includes(retired),false,retired);
...
test("minimal comparator parameters stay isolated from all canonical KV mechanics and benchmark methodology",async()=>{
  ...
  assert.deepEqual(Object.keys(comparators.damage).sort(),["battle_master","eldritch_knight"]);assert.equal(Object.hasOwn(comparators,"control"),false);
