...
test("Example Play and Glacial examples render exactly once in their destinations",async()=>{
  ...
  assert.equal(exampleArticle.querySelector(":scope > h2")?.textContent,"Example Play");assert.equal(exampleArticle.querySelector(":scope > .example-play-flow")?.className,"example-play-flow");assert.equal(exampleArticle.querySelector(".example-play-sections,.example-play-section__card"),null);assert.deepEqual(sections.map(section=>section.querySelector("h3")?.textContent),["Cryokinesis","Pyrokinesis","Psychokinesis","Electrokinesis"]);
  assert.deepEqual(sections.map(section=>section.querySelector("h4")?.textContent),["Example — Lockdown Turn, Level 11 Cryokinesis (PB 4, Int +4, MS 1d10, 3 attacks)","Example — Full Attack Turn, Level 11 Pyrokinesis (PB 4, Cha +4, MS 1d10, 3 attacks)","Example — Sustained Turn, Level 11 Psychokinesis (PB 4, Int +4, MS 1d10, 3 attacks)","Room Sweep — Electrokinesis"]);
  assert.deepEqual(sections.map(section=>section.querySelectorAll(".example-play-section__body > p").length),[5,5,6,5]);assert.match(sections[0]?.textContent??"",/Frozen Ground is already active.*Damage: 32\.5 cold/s);assert.match(sections[1]?.textContent??"",/You want focused damage.*47\.5 fire/s);assert.match(sections[2]?.textContent??"",/No need to nova.*30\.5 force/s);const roomSweep=sections[3]?.textContent??"";assert.match(roomSweep,/Attack 1:.*2×PB = 8.*up to PB \(4\) other creatures.*Charisma save.*not hit.*On a miss.*Blood Tax remains paid/s);assert.match(roomSweep,/Attack 2:.*Psi: 2.*1d10 lightning damage.*not hit.*On a miss.*2 Psi remains spent/s);assert.match(roomSweep,/Attack 3:.*Psi: 2.*10-foot-radius Sphere.*includes it.*2d8 lightning damage.*not hit.*On a miss.*2 Psi remains spent/s);assert.match(roomSweep,/Psi: 4\. Blood Tax: 8\. Three primary creatures are struck.*Branching Bolt and Electron Burst were each used once.*Only Static Discharge was Tier 2.*clear the room/s);
