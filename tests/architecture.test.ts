...
test("Markdown example sources have ordered, isolated authority placements",async()=>{
  ...
  const inline=glacial.content.find(block=>block.type==="example")!;const inlineMap=sourceMap.blocks.find((block:any)=>block.classification==="overload_inline_example")!;assert.equal(inline.tier,2);assert.deepEqual(nodes(inline).map((node:any)=>node.source_unit_id),inlineMap.source_unit_ids);for(const node of nodes(inline))assert.equal(node.text,visible(units.get(node.source_unit_id)!),node.source_unit_id);
  for(const mapped of sourceMap.blocks){const expected=mapped.classification==="overload_inline_example"?"glacial_spike":"common_example_play";for(const sourceId of mapped.source_unit_ids){const owners=authority.entities.filter(entity=>JSON.stringify(entity).includes(sourceId)).map(entity=>entity.id);assert.deepEqual(owners,[expected],`${sourceId} owner`);}}
