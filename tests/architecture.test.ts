...
test("retired migration sources are absent from the active architecture",async()=>{
  ...
  const productionFiles=(await Promise.all(["src","schema","policy",".github"].map(filesUnder))).flat();
...
test("prototype and release builds reflect direct YAML edits",async()=>{
  ...
  try{
    ...
    for(const result of [prototype,release]){const html=await readFile(result.htmlPath,"utf8");assert.match(html,/Kinetic Vanguard YAML Edit Probe/);assert.doesNotMatch(html,/Kinetic_Vanguard\.md|npm run migrate|edit (?:the )?Markdown/i);assert.match(html,/"rules_version":"14\.3\.0"/);assert.doesNotMatch(html,/application_version/);}
