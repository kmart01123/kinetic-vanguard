...
test("YAML authority is schema-valid, semantically valid, and complete",async()=>{
  ...
  assert.equal(loaded.authority.rules_version,"14.3.0");
...
test("prototype and release builds reflect direct YAML edits",async()=>{
  ...
  try{
    ...
    for(const result of [prototype,release]){assert.equal(result.manifest.build_identity.rules_version,"14.3.0");assert.equal("application_version" in result.manifest.build_identity,false);}
    const coverage=JSON.parse(await readFile(join(prototypeRoot,"coverage-ledger.json"),"utf8"));const {authority}=await loadAuthority(authorityPath);assert.equal(coverage.version,4);assert.equal(coverage.entity_count,44);assert.equal(coverage.entity_count,authority.entities.length);assert.deepEqual(coverage.entities.map((entity:any)=>entity.entity_id),authority.entities.map(entity=>entity.id));assert.ok(coverage.entities.every((entity:any)=>entity.content_block_count>0&&entity.destinations.length>0));assert.equal(coverage.entities.some((entity:any)=>entity.entity_id===authority.onboarding.id),false);
    const expectedSections=[authority.onboarding.blood_tax,authority.onboarding.disciplines,authority.onboarding.basic_turn,authority.onboarding.build_checklist,authority.onboarding.glossary,authority.onboarding.next_destinations].map(section=>section.id),expectedDestinations=[authority.onboarding.blood_tax,...authority.onboarding.primary_paths,...authority.onboarding.disciplines.cards,...authority.onboarding.basic_turn.destinations,...authority.onboarding.build_checklist.items,...authority.onboarding.glossary.entries,...authority.onboarding.next_destinations.items].map(item=>item.id);
    assert.deepEqual(coverage.onboarding,{authority_path:authorityPath+"#/onboarding",onboarding_id:authority.onboarding.id,section_ids:expectedSections,destination_ids:expectedDestinations});assert.equal(coverage.onboarding.section_ids.filter((id:string)=>id===authority.onboarding.blood_tax.id).length,1);assert.equal(coverage.onboarding.destination_ids.filter((id:string)=>id===authority.onboarding.blood_tax.id).length,1);assert.equal(new Set(coverage.onboarding.section_ids).size,coverage.onboarding.section_ids.length);assert.equal(new Set(coverage.onboarding.destination_ids).size,coverage.onboarding.destination_ids.length);
...
test("final rules decisions leave every unapproved authority field unchanged",async()=>{
  ...
  const masteryForIssue88=projection.entities.find((entity:any)=>entity.id==="common_kinetic_mastery");const glacialInteraction=masteryForIssue88.content.find((block:any)=>blockText(block).startsWith("Slow and Glacial Spike:"));
  assert.equal(glacialInteraction.inlines[0].text,"Slow and Glacial Spike: Declaring Glacial Spike replaces Slow for that hit. Resolve only Glacial Spike’s Speed effect for that Manifested Strike. Repeated applications of the same Speed reduction do not stack.");
  glacialInteraction.inlines[0].text="Slow and Glacial Spike: Slow and Glacial Spike are separate Speed reductions and stack with each other. If both apply, the target’s Speed is reduced by 20 feet until the start of your next turn. After Slow ends, Glacial Spike’s remaining 10-foot reduction continues until the end of that turn. Neither effect stacks with additional applications of itself.";
