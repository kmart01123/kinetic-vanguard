...
def load_config(path:Path=DEFAULT_CONFIG)->dict[str,Any]:
    ...
    damage=_object(data["damage_matrix"],"damage_matrix");_exact_keys(damage,{"non_damage_effect_boundary","optimization","excluded_stateful_features"},"damage_matrix")
...
def load_comparators(path:Path=DEFAULT_COMPARATORS)->dict[str,Any]:
    with path.open(encoding="utf-8") as stream:data=json.load(stream)
    data=_object(data,"comparator config");_exact_keys(data,{"format_version","source_ruleset","primary_comparator_ids","damage"},"comparator config")
