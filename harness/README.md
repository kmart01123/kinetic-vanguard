...
`harness.control_engine` is the narrow public facade over the one maintained computational runtime for control consequences, gates, overlap, state, and timing. That runtime is Python only: `control_catalog.py`, `control_graph.py`, `control_state.py`, `control_timeline.py`, and `control_engine.py`. TypeScript retains cheap architecture, manifest, and input-presence checks, but has no twin semantic evaluator or engine-parity corpus. The facade consumes the validated Control Authority v2.1 projection, the exact control-target supplement, the source-backed seven-condition consequence catalog (Blinded, Charmed, Frightened, Incapacitated, Prone, Restrained, and Stunned), and the separate `config/control-engine.json` methodology configuration.

The control engine, consequence catalog, primitive contract, normalization rules, timeline engine, and engine configuration are each version `2.0.0`. Engine results retain the authority projection version and digest, target-supplement digest, consequence-catalog version and digest, primitive-contract version, normalization-rules version, timeline-engine version, `engine_config_version` and `engine_config_digest`, selected initiative and area-response convention IDs and versions, and displacement-function ID and version. Consequence provenance pins only the accepted public issue #53 and #54 record IDs; it contains no private-source locator, page, or source hash. The configuration and runtime are project-authored methodology and software.

The supported mutable boundary is `ControlEngine.execution_session(...)`, and the supported final assembly boundary is `session.result()`. The v2 session accepts provenance-bearing initial condition instances rather than unique condition IDs, preserves independently sourced duplicate instances, and records generic parent/child inclusion lineage. A session binds the compiled effect and authority digest; engine, catalog, primitive, normalization, timeline, configuration, initiative, area, and displacement identities; exact target identities and relevant mechanics; selector membership and context; choices; probability context and kernel provenance; candidate components; the ordered reliability script; and the exact timeline schedule into a deterministic canonical scenario record and SHA-256 integrity digest. Independent mutable state, chronological helpers, and collection-based result assembly are not public facade operations.
...
Typed attack and saving-throw opportunities derive their roll mode from closed live condition state. Advantage and Disadvantage sources each collapse, opposing modes cancel, and automatic save failure is resolved before a roll is created. Any non-condition roll-modifier source IDs must be bound in the canonical scenario and exactly match the resolver input; final replay requires the exact union of those bound sources and the sources derived from live condition normalization. Charmed target legality is checked before attack, damaging-ability, or damaging-magical-effect resolution. Applying Incapacitated exposes Action, Bonus Action, Reaction, speech, Initiative, and airborne-fall facts and destructively ends the exact concentration effect owned by the affected actor; ending the condition does not restore that effect. Prone, Incapacitated, and a compiled live-state change that reaches Fly Speed 0 share one sourced current-position fall transition, with simultaneous triggers collapsed and hover or explicit prevention honored. Fall context and standalone source facts are likewise scenario-bound; replay re-executes the fall and reconstructs the pre/post movement state before accepting Fly-Speed-0 provenance.
