...
The generated control publication preserves README's Single-Target Damage front door and short control-detail link, while publishing the exhaustive control results, catalog, and methodology in [`../CONTROL_BENCHMARK_DETAIL.md`](../CONTROL_BENCHMARK_DETAIL.md) from the same validated control run. Catalog inventory comes from the canonical harness projection: stable entity ID, player-facing title, discipline membership, feature and Overload-tier availability, canonical damage tiers, control tiers, target-role semantics, and Mastery metadata. Advanced Training is excluded from this ordinary-discipline inventory. A canonical control-bearing form missing from the headline scenario set is evaluated only for publication through the existing scenario and primitive pipeline; it never enters headline winner selection or changes `config/benchmark.json`.
