...
Each session owns a monotonic event cursor. It explicitly advances to known schedule events, preserves caller order for multiple operations at one event, captures pre-event plus per-operation pre/post state, and closes the event before further advancement. Expiry, branches, replacement/refresh, normalization, Prone and area responses, concentration, displacement, and epoch changes issue typed records carrying the session digest and schedule identity. Earlier, unknown, future-without-advance, foreign-session, stale, fabricated, or inactive-source records are rejected rather than repaired by final sorting.
