...
On-hit riders use the canonical `per_manifested_strike` repeatability contract. The damage planner can select the same rider again on later strikes, and the Control Reliability harness can retry a fixed Tier 0 or Tier 1 rider after an observed miss or failed control attempt while attacks, Psi, and Blood Tax budget remain. Tier 2 remains limited to one declaration per Attack action.
