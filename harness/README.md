...
Battle Master and Eldritch Knight consume only their declared comparator configuration. Fighter attack progression, Studied Attacks, Combat Prowess, maneuver timing, Relentless, Hew, True Strike, weapon packages, and source-backed resistance handling are evaluated analytically from current code and configuration.
...
The Eldritch Knight shadow inventory accounts for the 41 retained audited spells without duplicating the 110 exclusions in runtime data. Exact finite save penalties and deterministic turn branches are analytically enumerated. Area occupancy, two-sided visibility or isolation, action escapes, transformations, and other context-dependent packages remain explicitly unpriced where the maintained benchmark lacks the required geometry, opportunity, environment, behavior, or replacement-form data.
