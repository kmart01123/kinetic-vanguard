...
The generated control publication keeps the compact comparator-band tables, adds raw Kinetic Vanguard companion tables, and publishes an exhaustive exact-form catalog from the same validated control run. Catalog inventory comes from the canonical harness projection: stable entity ID, player-facing title, discipline membership, feature and Overload-tier availability, canonical damage tiers, control tiers, target-role semantics, and Mastery metadata. Advanced Training is excluded from this ordinary-discipline inventory. A canonical control-bearing form missing from the headline scenario set is evaluated only for publication through the existing scenario and primitive pipeline; it never enters headline winner selection or changes `config/benchmark.json`.

Each catalog cell uses the complete maintained level roster. Ineligible targets contribute zero CU and zero delivery while remaining in the denominator, and coverage reports eligible / total targets. Pricing state comes from retained normalized primitive rows rather than `CU == 0`: priced forms retain scalar candidates only, partial forms also retain a distinct context-required or unsupported consequence, and unpriced forms retain only context-required or unsupported consequences. Suppressed duplicate or weaker primitives cannot create a false partial result. Canonical tiers with no declared control remain distinct as `no modeled control`, while unavailable features or locked tiers render as `N/A`.
