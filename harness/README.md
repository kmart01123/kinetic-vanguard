...
A failed concentration check is a two-event public-session lifecycle. At the typed damage-context event, the session validates the exact check evidence, nonzero selected probability, immediate typed end binding, compiled end plan, affected targets, and any target-scoped normalization barrier before issuing an immutable session-owned pending failure. The tracker remains active with exactly one new check record, while components, ambient area state, membership, and routes remain unchanged. Only the immediately following bound `concentration_end` event may consume that pending identity: it ends the tracker once, executes the compiled end gates and current-position falls, terminates concentration and area-bound components, closes membership and routes, and clears the pending state. Final replay proves the same-session and same-scenario pair, adjacent event IDs and sequences, continuous tracker state, unchanged damage-event component and route state, exact end plan and gates, end-only cleanup, and an empty pending lifecycle; missing, duplicate, foreign, stale, or rewritten pairs fail closed. This correction does not advance the `1.0.0` engine, catalog, primitive-contract, normalization, timeline, or configuration versions.
