...
Typed attack and saving-throw opportunities derive their roll mode from closed live condition state. Advantage and Disadvantage sources each collapse, opposing modes cancel, and automatic save failure is resolved before a roll is created. Charmed target legality is checked before attack, damaging-ability, or damaging-magical-effect resolution. Applying Incapacitated exposes Action, Bonus Action, Reaction, speech, Initiative, and airborne-fall facts and destructively ends the exact concentration effect owned by the affected actor; ending the condition does not restore that effect. Prone, Incapacitated, and a compiled live-state change that reaches Fly Speed 0 share one sourced current-position fall transition, with simultaneous triggers collapsed and hover or explicit prevention honored.
