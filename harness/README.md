...
- Eldritch Knight control keeps one structured SRD-only scenario inventory. The legacy/default Control Reliability selection consumes only `reliability_scenario_ids`, which remain plain Blindness/Deafness and Blindness after Eldritch Strike. Optional Stage-A shadow packages additionally consume spell level/access, delivery, spell-attack or save math, duration/repeat timing, condition/outcome, creature-type eligibility, and magnitude metadata. Spell attacks use proficiency plus the single maintained spellcasting-ability progression; weapon ability and magic bonuses apply only to weapon attacks and the prior-Attack-action Eldritch Strike primer. Primer success remains at least one hit, repeated hits do not stack beyond Disadvantage, and neither Action Surge, same-turn Improved War Magic primers, repeated casts, nor a spell-resource optimizer is modeled.
