...
The generated publication keeps README as the Single-Target Damage front door with one short control-detail link. Exhaustive control results, the exact-form catalog, effective coverage, comparator references, and methodology live in [`../CONTROL_BENCHMARK_DETAIL.md`](../CONTROL_BENCHMARK_DETAIL.md), generated from the same validated control run. Catalog inventory comes from the canonical harness projection: stable entity ID, player-facing title, discipline membership, feature and Overload-tier availability, canonical damage tiers, control tiers, target-role semantics, and Mastery metadata. Advanced Training is excluded from this ordinary-discipline inventory. A canonical control-bearing form missing from the headline scenario set is evaluated only for publication through the existing scenario and primitive pipeline; it never enters headline winner selection or changes `config/benchmark.json`.

Structural eligibility is internal prerequisite evidence derived from maintained size, creature-type, and other structural target rules. Public effective coverage instead reports effective targets / total roster targets: a target is effective when at least one modeled consequence from that exact source survives structural restrictions, immunities, and dependencies. Partially effective targets remain in the numerator; fully nullified and structurally excluded targets do not. CU and delivery still use the complete roster denominator, so public effective coverage never creates an effective-only averaging population. See the [exhaustive reader-facing methodology](../CONTROL_BENCHMARK_DETAIL.md) for the generated evidence and examples.

Pricing state comes from retained normalized primitive rows rather than `CU == 0`: priced forms retain scalar candidates only, partial forms also retain a distinct context-required or unsupported consequence, and unpriced forms retain only context-required or unsupported consequences. Suppressed duplicate or weaker primitives cannot create a false partial result. Canonical tiers with no declared control remain distinct as `no modeled control`, while unavailable features or locked tiers render as `N/A`.

The Sap and Stunned arithmetic is resolved from the maintained primitive catalog and frozen scoring config during rendering, so catalog or weight drift fails the publication contracts instead of leaving hand-maintained examples behind.

The writer atomically synchronizes the delimited README balance region and the root-level detail page, and refuses to overwrite either after a concurrent change. Use it only after an intentional authority, methodology, roster, comparator, or release-state change and review the numerical diff.
...
Holdout uses its canonical level-banded strike formula: levels 3–17 halve the complete strike packet and Fighter 18+ uses the Refined Holdout 1d6 + Psionic Ability modifier packet.

At Fighter 18+, Psionic Apex grants the same 3d8 native-damage Discipline Maturation packet to all four disciplines: cold for Cryokinesis, fire for Pyrokinesis, force for Psychokinesis, and lightning for Electrokinesis. Availability is once per Attack action, and each new Attack action, including one from Action Surge, starts with a fresh use; later attacks within the same Attack action do not refresh it. This separate packet damages only the struck creature, costs no Psi or Blood Tax, does not double on a critical hit, and has no Overload tier. Resistance and Immunity apply normally even with a Tier-2 rider or Holdout.

The surgical damage integration keeps Absolute Zero's six dice and fixed bonus in one exact distribution before save halving and defense rounding. Telekinetic Shove resolves its canonical fixed-damage threshold at the current Fighter level. See [the earlier surgical integration](../docs/surgical-damage-integration.md).

Branching Bolt exposes canonical `damage_allocation` at every tier from its Fighter-7 grant (Tier 2 remains gated to Fighter 10). The native planner declares a full 3/4/5-die allocation before the original strike. Each declaration requires the primary plus at least two distinct hostile secondaries, with at least one die per target. The planner offers no Branching Bolt package in clusters smaller than three. Its homogeneous roster assumes all recipients are hostile; mixed allegiance is outside the benchmark. The allocation contract carries `secondary_target_eligibility: hostile`, and declaration validation rejects any non-hostile secondary. It enumerates every legal distribution within the tier's capacity, including extra dice on either the primary or a secondary, consolidating equivalent secondary permutations while preserving primary identity.

An original miss cancels the rider; an original hit lands every allocated die without additional attack rolls. The original critical doubles base strike dice only, with no rider doubling or bonus die. Allocated dice form one defended packet per target; only Tier 2 bypasses resistance, and immunity still applies. Base strike and Maturation remain separate. Studied Attacks and Combat Prowess use the existing original-strike logic; branches create no secondary attack state or extra Prowess opportunity. Policy labels expose allocations, for example `branching_bolt@3+1+1:T2`. Projection is 1.11.0, schema 2.15.0, rules 15.0.0. See [integration evidence](../docs/branching-bolt-integration.md).

For all four disciplines, players must predeclare Discipline Maturation before the attack roll; the once-per-Attack-action use is spent on that declared attack even if it misses. It is a gamble, not an automatic on-hit proc or a choice made after seeing the roll. The planner chooses whether to declare or defer using only observed state before enumerating roll outcomes. Only a hit by the declared Manifested Strike qualifies for the extra damage, including a miss converted to a hit by Combat Prowess. An undeclared hit does not consume the use. A declared miss spends it without delivering the packet, and a later hit cannot recover that spent use. There is no automatic first qualifying hit consumption. Immunity can reduce a declared hit's packet to zero without refunding the use.
...
Fighter attack progression, Studied Attacks, Combat Prowess, Relentless, True Strike, weapon packages, and source-backed resistance handling are evaluated analytically from current code and configuration. An Action Surge slot can supply another Attack action but never an Eldritch Knight Magic action, and it can supply another eligible standalone Kinetic Vanguard psionic Action. Each activation independently pays its Psi and Blood Tax; concentration still permits only one active feature.
...
Control Reliability's `Whole-package control stick %` is the probability that the selected exact source/package establishes at least one modeled control consequence in its legal initial delivery window, after all maintained legal retries and resource constraints are applied. It does not compare condition severity, duration, area, or strategic value. For each target, build, and discipline, the harness first filters out ineligible scenarios and selects the legal package with the highest Control Value. An exact CU tie is resolved by higher whole-package Reliability, then by lexicographically ascending stable scenario ID. Reliability reports initial establishment/delivery for that same CU-selected winner; it never runs a second winner-selection pass. `Still controlled after configured repeats %` remains a separate persistence diagnostic, not headline Reliability.
