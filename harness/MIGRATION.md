...
The old files remain historical evidence through Git and frozen v14.1 refs. They are not copied, aliased, dual-read, or retained as a golden. Maintained work uses `creature_catalog.py`, `data/srd_creatures.json`, `data/srd_creature_rosters.json`, `config/creature-consumers.json`, and `provenance/srd-creatures.json`; live scenario state stays outside those static records.
