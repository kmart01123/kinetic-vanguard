...
- Advanced the damage input/result provenance contract to `2.0.0` and the control-engine public result contract to `3.0.0` for explicit catalog, whole-roster, active-profile, projection, and consumer-scoped requirement identities. The whole requirements-registry digest remains repository-integrity metadata, while damage and control results bind only their precise consumer digests. Control-engine mechanics remain `2.0.0`.
...
- Marked the single pre-correction 47-target v14.2 exact damage run `invalidated_premerge_provenance_boundary_correction` after correcting the consumer-scoped requirements and sibling-projection implementation identities. Its original manifest and output hashes, synchronized table, and compact v14.1 delta remain permanent comparison evidence; no numerical defect was demonstrated and no corrected-contract replacement run has yet been performed.
