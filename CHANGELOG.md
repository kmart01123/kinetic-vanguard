...
- Advanced the shared control engine, consequence catalog, primitive contract, normalization rules, timeline engine, and engine configuration to `2.0.0`. The successor session replaces unique/self-sourced initial conditions with provenance-bearing condition instances and generic inclusion lineage; makes Prone remain, stand, voluntary drop, and crawl explicit proposals; evaluates Charmed legality and attack/save modes against live source-relative state; and couples Incapacitated to exact concentration cleanup, action/Initiative facts, and sourced current-position falls, including state-derived Fly Speed 0. External roll-modifier sources and fall facts are scenario-bound, and final replay exact-checks their union with live condition sources plus the pre/post movement transition. Control Authority remains `2.1.0`, the canonical schema remains `3.1.0`, Kinetic Vanguard rules remain `14.2.0`, and damage contracts are unchanged.
