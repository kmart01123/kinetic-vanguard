...
- Added one provenance-bound 330-record SRD 5.2.1 creature catalog, complete source/candidate accounting, deterministic source-mechanical-diversity headline and eligible-census profiles, a closed passive-trait registry, consumer requirements, and independent thin `DamageTarget`/`ControlTarget` projections.
...
- Advanced the damage input/result provenance contract to `2.0.0` and the control-engine public result contract to `3.0.0` for explicit catalog, whole-roster, active-profile, projection, and consumer-requirement identities. Control-engine mechanics remain `2.0.0`.
...
- Recorded the single frozen 47-target v14.2 exact damage run, synchronized the README from its manifest, and added a compact provenance-bound mechanical delta audit against the accepted v14.1 release matrix without claiming independent certification.
- Advanced the shared control engine, consequence catalog, primitive contract, normalization rules, timeline engine, and engine configuration to `2.0.0`. The successor session replaces unique/self-sourced initial conditions with provenance-bearing condition instances and generic inclusion lineage. Every separately issued condition application retains an independent lifecycle, including applications with the same source actor, program, effect, component, and invocation when their issuance identities differ, while identical derived consequences remain nonstacking. The session makes Prone remain, stand, voluntary drop, and crawl explicit proposals; evaluates Charmed legality and attack/save modes against live source-relative state; and couples Incapacitated to exact concentration cleanup, action/Initiative facts, and sourced current-position falls, including state-derived Fly Speed 0. External roll-modifier sources, fall facts, opportunity context, standalone save ability, and source-relative action proposals are scenario-bound. Final replay re-executes condition applications, exact ends, invocation-owned compiled branches, and duration expiry; reconstructs live normalization and pre/post movement state; binds each fall trigger to its current operation; and exact-checks typed-event actors, one roll record per typed opportunity and sequentially reachable compiled attack/save gate including denied attacks, and one consuming branch transition for every legal, no-op, or nested concentration-end resolution. Control Authority remains `2.1.0`, the canonical schema remains `3.1.0`, Kinetic Vanguard rules remain `14.2.0`, and damage contracts are unchanged.
