...
- Made shortest-route geometry, live area membership, and exact remaining distance session-owned across movement opportunities; membership now drives movement and compiled recurring-area gate eligibility even with no active area component, exits and canonical re-entries update later gates, raw per-event route replacement is rejected, and typed moving-area transitions retain deterministic route-state snapshots and hashes. Zero-area programs accept either inert area convention, while multiple-area ambiguity fails closed.
